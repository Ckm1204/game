...
export default class World {
    ...
    async loadLevel(level, startPosition = { x: 0, y: 5, z: 0 }) {
        try {
            const backendUrl = import.meta.env.VITE_BACKEND_URL || 'http://localhost:3001';
            const apiUrl = `${backendUrl}/api/blocks?level=${level}`;

            // 🔵 Cargar bloques del backend
            await this.loader.loadFromURL(apiUrl);
            console.log(`✅ Nivel ${level} cargado.`);

            // 🟢 Inicializar robot en la posición correcta
            this.robot = new Robot(
                this.experience,
                new CANNON.Vec3(startPosition.x, startPosition.y, startPosition.z)
            );
        } catch (error) {
