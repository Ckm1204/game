...
export default class World {
  ...
  update(delta) {
      ...
      if (dist < 1.2 && moved && !prize.collected) {
        ...
        if (prize.role === "default") {
          ...
          if (!this.finalPrizeActivated && this.points === pointsTarget) {
            const finalCoin = this.loader.prizes.find(
              (p) => p.role === "finalPrize"
            );
    ...
    if (!this.finalPrizeActivated && this.loader?.prizes) {
      const totalDefault = this.loader.prizes.filter(
        (p) => p.role === "default"
      ).length;
      const collectedDefault = this.loader.prizes.filter(
        (p) => p.role === "default" && p.collected
      ).length;
  ...
  async loadLevel(level) {
    try {
      ...
      try {
        ...
        console.warn(
          `⚠️ No se pudo conectar con el backend. Usando datos locales para nivel ${level}...`
        );
...
this.experience.physics.world.addEventListener('beginContact', (event) => {
    ...
    if (ids.includes(robotBodyId) && ids.includes(foxBodyId)) {
      ...
      this.totalDefaultCoins = this.loader.prizes.filter(
        (p) => p.role === "default"
      ).length;
      console.log(
        `🎯 Total de monedas default para el nivel ${level}: ${this.totalDefaultCoins}`
      );
  ...
  async _processLocalBlocks(blocks) {
    ...
    console.log(
      `🎯 Total de monedas default para el nivel local: ${this.totalDefaultCoins}`
    );
  }
